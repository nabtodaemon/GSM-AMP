...
void nabto_message_event(message_event* event, uint16_t ilen) {
    ...
    if (event->type == MT_UDP) {
		...
		printf("Unabto_massage_base_station -%d %d %d %d\n", event->udpMessage.peer.addr, event->udpMessage.peer.port, nmc.controllerEp.addr, nmc.controllerEp.port);
		printf("Unabto_massage_base_station_gsp -%d %d %d %d\n", event->udpMessage.peer.addr, event->udpMessage.peer.port, nmc.context.gsp.addr, nmc.context.gsp.port);
        if (fromBS) {
